...
import { ChatMessage, ReactionType, ServerPayload, initReactions, reactionButtons } from '../types/chat';

const fallbackMessages: ChatMessage[] = [
  { id: 'local-1', sender: 'Anon-84X', text: 'Welcome to the hidden group 👋', sentAt: '10:14', kind: 'text', reactions: initReactions() },
  { id: 'local-2', sender: 'Anon-Q7B', text: 'Drop your mood sticker below 🎉', sentAt: '10:15', kind: 'text', reactions: initReactions() },
  { id: 'local-3', sender: 'Anon-D11', text: 'New GC? Yes 🔥', sentAt: '10:16', kind: 'text', reactions: initReactions() },
];
...
export default function GroupChatPage() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (payload.type === 'message') setMessages((prev) => [...prev, payload.message]);
        if (payload.type === 'reaction') setMessages((prev) => prev.map((msg) => (msg.id === payload.message.id ? payload.message : msg)));
      } catch {
  ...
  const publishMessage = (message: ChatMessage) => {
    if (!sendSocketPayload({ type: 'message', message })) setMessages((prev) => [...prev, message]);
  };
  ...
  const reactToMessage = (messageId: string, reaction: ReactionType) => {
    ...
    setMessages((prev) =>
      prev.map((msg) => {
        ...
        if (current === reaction) delete next.byUser[youAlias];
        else {
          next[reaction] += 1;
    ...
    <main className="chat-page-wrap">
      <section className="chat-shell">
        <div className="chat-appbar">
          <button className="chat-icon-circle" type="button" aria-label="Back">←</button>
        ...
        <div className="pinned-row">
          <span>📌</span>
          <p>@all keep it kind, keep it honest.</p>
        </div>

        <div className="chat-box whatsapp-chat" ref={chatListRef}>
          {messages.map((msg) => {
            const isMe = msg.sender === youAlias;
            const hasReactions = msg.reactions.love + msg.reactions.like + msg.reactions.angry > 0;
              ...
              <article key={msg.id} className={`chat-bubble ${isMe ? 'me' : 'other'}`}>
                ...
                {reactionMenuFor === msg.id ? (
                  <div className="reaction-popover">
                    {reactionButtons.map((item) => (
                      <button key={item.type} type="button" onClick={() => reactToMessage(msg.id, item.type)}>
                ...
                {hasReactions ? (
                  <div className="reaction-summary">
                    {msg.reactions.love > 0 ? <span>❤️ {msg.reactions.love}</span> : null}
                    {msg.reactions.like > 0 ? <span>👍 {msg.reactions.like}</span> : null}
                    {msg.reactions.angry > 0 ? <span>😡 {msg.reactions.angry}</span> : null}
                  </div>
                ) : null}
              </article>
            );
        ...
        <div className="chat-composer">
          <form onSubmit={onSend} className="composer-form">
            <div className="composer-main">
              ...
              <textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Message" rows={1} />

              <button className="icon-btn" type="submit" aria-label="Send message">📤</button>
            </div>
